...
for file in os.listdir(original_path):
    ...
    line3 = []
    for i in range(0,5):
        input = imgs[i] # torch.tensor(3, 256, 256)

        f_input = dwt(input)
        f_output = idwt(f_input)
        f_input = Split(f_input)
        line2.append(f_input)
        line3.append(f_output)

    con = torch.cat(line2, dim = 2)
    con2 = torch.cat(line3, dim = 2)
    print((con2 - original).max())
    ...
    cv2.imwrite(os.path.join(target, '_gray' + file), result)
